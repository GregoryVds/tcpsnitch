...
#define DEBUG(debug_level, format, args...) {\
	...
	lib_log(debug_level, formated_string);\
}
...
void addr_string_from_sockaddr(const struct sockaddr_storage *addr, char *buf, int buf_size);
void port_string_from_sockaddr(const struct sockaddr_storage *addr, char *buf, int buf_size);
